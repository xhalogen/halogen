...
mod tests {
    ...
    fn matmul_2x2() {
        ...
        let c: DenseTensor<i32> = crate::einsum!(a[i, j] * b[j, k] => [i, k]);
    ...
    fn matvec() {
        ...
        let w: DenseTensor<i32> = crate::einsum!(a[i, j] * v[j] => [i]);
        assert_eq!(w.shape(), &[2]);
    ...
    fn dot_product() {
        ...
        let c: DenseTensor<i32> = crate::einsum!(a[i] * b[i] => []);
        // 1*4 + 2*5 + 3*6 = 32
    ...
    fn outer_product() {
        ...
        let c: DenseTensor<i32> = crate::einsum!(a[i] * b[j] => [i, j]);
