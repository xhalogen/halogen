...
pub fn gen_size_checks(
    ...
    for (i, t) in tensors.iter().enumerate() {
        let shval = format_ident!("__halogen_einsum_tensor{i}");
...
pub fn def_outputvalues(
    ...
    ret.push(quote! {
        let __halogen_einsum_output_data: usize = __halogen_einsum_output_shape.iter().product();
...
pub fn gen_run_einsum(
    ...
    quotes = quote! {
        ...
        let mut __halogen_einsum_iter_stride: usize; // init stride
        #quotes
        let __halogen_einsum_output_tensor = halogen::core::tensor::Tensor::from_vec(
